...
const FoodList = () => {
  ...
  return (
    <div className="flex flex-col items-center bg-gray-50 min-h-screen py-6">
      ...
      <div className="flex justify-between items-center bg-gray-100 w-4/5 p-4 rounded-xl shadow-lg mb-8">
      <img src={Logo} alt="" className='w-20 h-20' />
        <p className="text-3xl font-extrabold ">Good Food</p>
        <div className="flex items-center bg-white p-2 rounded-xl shadow-md">
          ...
          <p className="bg-red-500 text-white rounded-full h-6 w-6 flex items-center justify-center">{cart.length}</p>
        </div>
      </div>
      ...
      <div className="flex w-4/5 gap-6">
        ...
        <div className="grid grid-cols-3 gap-6 w-full">
          {products.map((product) => (
            <div
              key={product.id}
              className="border border-black p-6 rounded-xl flex flex-col items-center bg-white shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200 cursor-pointer"
            >
              <div className="h-44 w-full flex items-center justify-center rounded-xl overflow-hidden">
                <img
                  src={product.image}
                  alt={product.title}
              ...
              <div className="mt-4 flex flex-col items-center text-center">
                ...
                <button
                  ...
                  className="bg-green-600 text-white py-2 px-6 rounded-md mt-4 hover:bg-green-700 transition duration-300 transform hover:scale-105"
                >
                  Add to Cart
                </button>
              </div>
        ...
        {/* Cart Sidebar */}
        {isCartVisible && (
          <div className="w-2/5 max-w-sm border p-6 rounded-xl bg-white shadow-lg">
            <div className="bg-gray-200 p-3 rounded-xl mb-6 text-center">
            ...
            <div>
              {cart.length === 0 ? (
                ...
                cart.map((item, index) => (
                  <div
                    key={index}
                    className="flex justify-between items-center bg-gray-100 p-3 rounded-lg mb-4 shadow-sm"
                  >
                    <div className="flex items-center">
                      ...
                      <p className="text-sm ml-4 text-gray-700">{item.title}</p>
                    </div>
                    <div className="flex items-center">
                      <h2 className="text-lg text-gray-800">${item.price}.00</h2>
                      <i
            ...
            <div className="flex justify-between mt-4 pt-4 border-t border-gray-300">
              ...
              <h2 className="text-xl font-bold text-gray-900">${total}.00</h2>
            </div>
          </div>
        )}
      </div>
